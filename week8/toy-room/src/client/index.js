...
const socket = io('/');
const cameraPeer = new Peer(undefined, { host: '/', path: '/peerjs', port: 3001 });
const screenPeer = new Peer(undefined, { host: '/', path: '/peerjs', port: 3001 });
...
socket.on('chat message', ({ userId, msg }) => {
  ...
  const userName = state.otherUserList.find((user) => user.userId === userId)?.userName;
...
cameraPeer.on('call', (call) => {
  ...
  const user = state.otherUserList.find(user => user.cameraPeerId === cameraPeerId);
...
screenPeer.on('call', (call) => {
  ...
  const cameraPeerId = call.peer;
  const user = state.otherUserList.find(user => user.screenPeerId === cameraPeerId);
...
document.getElementById('share-video-btn').addEventListener('click', async () => {
  myCameraStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
